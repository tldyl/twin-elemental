    ...
    public void onEnterStance() {
        ...
        if (p instanceof ZetsuEnnNoTwins) {
            if (CharacterSelectScreenPatch.reskinIndex == 0) {
                AbstractDungeon.player.state.setAnimation(0, "ice_change_fire", false);
                AbstractDungeon.player.state.addAnimation(0, "fire_idle", true, 0.0F);
            } else {
                if (AbstractDungeon.player.img != null) {
                    AbstractDungeon.player.img.dispose();
                }
                AbstractDungeon.player.img = new Texture(TwinElementalMod.getResourcePath("char/reskin_esaw.png"));
            }
        }
